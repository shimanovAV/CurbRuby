...
class BetsControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should create bet' do
    assert_difference('Bet.count') do
      post bets_url, params: { chosen_bet_by_curb: { name: @bet.name, photo_url: @bet.photo_url } }
    end
  ...
  test 'should update bet' do
    patch bet_url(@bet), params: { chosen_bet_by_curb: { name: @bet.name, photo_url: @bet.photo_url } }
